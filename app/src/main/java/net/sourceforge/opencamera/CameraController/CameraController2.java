...
public class CameraController2 extends CameraController {
	...
	public void setExpoBracketingNImages(int n_images) {
		...
		if( n_images <= 1 ) {
			if( MyDebug.LOG )
				Log.e(TAG, "n_images should be greater than 1");//TODO Log.e(TAG, "n_images should be an odd number greater than 1");
			throw new RuntimeException(); // throw as RuntimeException, as this is a programming error
		}
	...
	private void takePictureBurstExpoBracketing() {
		if( MyDebug.LOG )
			Log.d(TAG, "takePictureBurstExpoBracketing");
		...
		try {
			...
			if( expo_bracketing_n_images == 2 ){
				long exposure_time = base_exposure_time;
				exposure_time /= scale;
				stillBuilder.set(CaptureRequest.SENSOR_EXPOSURE_TIME, exposure_time);
				stillBuilder.set(CaptureRequest.FLASH_MODE, CameraMetadata.FLASH_MODE_SINGLE);
				requests.add( stillBuilder.build() );
