...
public class HDRProcessor {
	 ...
	 */
	public void processHDR(List<Bitmap> bitmaps, boolean flash_NoFlash) {
		if( MyDebug.LOG )
			Log.d(TAG, "processHDR");
		int n_bitmaps = bitmaps.size();
		if( n_bitmaps != 3 && n_bitmaps != 2  ) {
			if( MyDebug.LOG )
				Log.e(TAG, "n_bitmaps should be 2 or 3, not " + n_bitmaps);
			// throw RuntimeException, as this is a programming error
			throw new RuntimeException();
		}
...
//			throw new RuntimeException();
	...
	private void processHDRAverage(List<Bitmap> bitmaps) {
        ...
        // set allocations
		processHDRScript.set_flashBuffer(allocations[0]);
		//processHDRScript.forEach_initFlashBuffer(allocations[0]);
		processHDRScript.set_noFlashBuffer(allocations[1]);
        ...
        if( MyDebug.LOG )
            Log.d(TAG, "call processHDRScript");
        processHDRScript.forEach_bilateral(allocations[0], allocations[0]);
        if( MyDebug.LOG )
            Log.d(TAG, "time after processHDRScript: " + (System.currentTimeMillis() - time_s));
